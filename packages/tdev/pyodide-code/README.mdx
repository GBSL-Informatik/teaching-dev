...
Kopiere das `packages/tdev/pyodide-code`-Verzeichnis in dein `tdev-website/website/packages`-Verzeichnis oder füge es dem `updateTdev.config.yaml` hinzu, damit es bei Updates nicht überschrieben wird.
:::


import pyodideSW from '!!raw-loader!@site/static/pyodide.sw.js';

1. Füge das `pyodide-code`-Package zu den `apiDocumentProviders` in deiner `siteConfig.ts` hinzu:
