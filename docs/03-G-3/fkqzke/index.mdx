...
In unserem Rätsel geht es um ein Gedankensystem, das von Hilbert geschaffen wurde. Stell Dir sich ein Hotel vor, welches **unendlich viele Zimmer** hat und **vollständig besetzt** ist. Was kann die Rezeption tun, wenn ein neuer Gast ein Zimmer beziehen möchte?
...
<AdventureOptions>
  ...
  <AdventureOption label="III" nextGuessIn="60">
    ...
    $$
    \begin  {aligned}
        n :\quad &\text{Zimmernummer}\\
        k : \quad &\text{ist ein Element der Natürlichen Zahlen}\\
        &\\
         \quad &\text{Jeder Gast geht ins folgende Zimmer}: n^k\\
        &\\
       & \rightarrow \text{Die neuen Gäste gehen in die jetzt freien Zimmer}\quad\\
    \end{aligned}
    $$
  ...
  <AdventureOption label="IV" nextGuessIn="60">
    ...
    \begin{aligned}
       & n: \text{Zimmernummer}\\
       & k: \text{ist ein Element der Natürlichen Zahlen}\\
        \\
       & \text{Die Gäste gehen in folgendes Zimmer}:\quad 3n\\

        &\quad \rightarrow \text{Die neuen Gäste gehen in die nun freien Zimmer}\\
    \end{aligned}
    $$
