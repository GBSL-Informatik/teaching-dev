---
sidebar_position: 1
page_id: c0670b9b-7ba1-42a9-b0bb-db35e8c54ff2
---

import { AdventureOptions, AdventureOption } from '@tdev-components/Adventure'

import Board from './Checkerboard'

:::tip[Wer bin ich?]
> In jedem Rätsel gibt es die Rubrik "Wer bin ich?", die im letzten Rätsel aufgelöst wird. Merk dir die Infos also gut.

Wie die Damen im Schach den König beschützen, engagierte ich mich im 2.Weltkrieg, indem ich Forschungen zu Kohlenutzungseffizienz machte, Luftschutzwartin war und mich bei Hilfsorganisationen betätigte, um meinem Land zu dienen… Wer bin ich?
:::

# N-Damen-Problem

> von Aviva und Sanja


## Was ist das Damenproblem? [^1]

Bei dem Damenproblem handelt es sich um ein mathematisches Problem, dass auf den bestehenden Schachregeln beruht.

Gegeben ist ein quadratisches Schachbrett, das mindestens n > 3 Felder gross sein muss. Auf diesem wird nun in jeder Spalte eine Dame platziert. Diese Damen müssen so platziert werden, dass sie sich horizontal, vertikal oder diagonal beliebig weit bewegen können, ohne sich gegenseitig zu schlagen. Das bedeutet, die platzierten Damen dürfen nicht in der gleichen Reihe, der gleiche Spalte und der gleichen Diagonalen stehen.

![Bewegungen der Dame beim Schach --width=424](./images/schachbrett-dame-laufwege.jpeg)

Für ein n x n Schachbrett muss man n Damen platzieren. Eine mögliche Lösung für ein 8 x 8 Schachbrett ist:&#x20;

![--width=425](./images/Moegliche_Loesungen.jpeg)

## Rätsel

Auf dem Schachbrett sind bereits 4 Damen platziert. Platziere die restlichen 4 Damen so, dass sie den Anforderungen des Damenproblems entsprechen. Kontrolliere deine Lösung mit der gegebenen Nummer.

<div style={{display: 'flex', alignContent: 'center', justifyContent: 'center'}}>
  <Board initialQueens={[{ row: 0, col: 6 },{ row: 3, col: 2 },{ row: 4, col: 0 },{ row: 7, col: 4 }]} />
</div>

<AdventureOptions>
  <AdventureOption label="1" nextGuessIn={60}>
    Antwort

    ---

    Ob deine Antwort richtig ist, findest du heraus, wenn du diesen QR-Code scannst.&#x20;

    ![--width=391](./images/Image1.jpeg)
  </AdventureOption>

  <AdventureOption label="2" nextGuessIn={60}>
    Antwort

    ---

    Ob deine Antwort richtig ist, findest du heraus, wenn du diesen QR-Code scannst.&#x20;

    ![--width=446](./images/QRCode.jpeg)
  </AdventureOption>

  <AdventureOption label="3" nextGuessIn={60}>
    Antwort

    ---

    Ob deine Antwort richtig ist, findest du heraus, wenn du diesen QR-Code scannst.&#x20;

    ![--width=420](./images/Image3.jpeg)
  </AdventureOption>

  <AdventureOption label="4" nextGuessIn={60}>
    Antwort

    ---

    Ob deine Antwort richtig ist, findest du heraus, wenn du diesen QR-Code scannst.&#x20;

    ![--width=406](./images/Image4.jpeg)
  </AdventureOption>

  <AdventureOption label="5" nextGuessIn={60}>
    Antwort

    ---

    Ob deine Antwort richtig ist, findest du heraus, wenn du diesen QR-Code scannst.&#x20;

    ![--width=416](./images/Image5.jpeg)
  </AdventureOption>
</AdventureOptions>

[^1]: Quelle: [https://l1nq.com/zvlOC](https://l1nq.com/zvlOC)\&#x20)
