...
<Steps>
  1. Eine neue App __<Val name="app" />__ auf dem Server erstellen
      <DynamicInput name="app" default='inf-teaching-api' placeholder='inf-teaching-api' />
      <DynamicInput name="domain" default='teaching-api-xxx.gbsl.website' placeholder='teaching-api-xxx.gbsl.website' />
      <DynamicInput name="APP_NAME" default={(page) => _.camelCase((page.dynamicValues.get('app') || 'inf-teaching-api'))} />
      <DynamicInput name="ALLOWED_ORIGINS" default='gbsl.website' placeholder='gbsl.website' />
      ...
      <DynamicInput name="MSAL_CLIENT_SECRET" monospace placeholder='client-secret' />
      ...
      <TemplateCode>
        ...
        dokku config:set --no-restart {{app}} BETTER_AUTH_SECRET="{{BETTER_AUTH_SECRET}}"
        dokku config:set --no-restart {{app}} BETTER_AUTH_URL="https://{{domain}}"
        dokku config:set --no-restart {{app}} MSAL_CLIENT_ID="{{MSAL_CLIENT_ID}}"
        dokku config:set --no-restart {{app}} MSAL_CLIENT_SECRET="{{MSAL_CLIENT_SECRET}}"
      ...
      Im Azure-Portal muss zusätzlich eine Umleitungs-URL des Type __Web__ mit <Val as="code">`{{BETTER_AUTH_URL}}/api/auth/callback/microsoft`</Val> hinterlegt werden.
  ...
  3. Optionale Umgebungsvariablen setzen:
        ...
        <Val as="code">`https://deploy-preview-\d+--{{NETLIFY_PROJECT_NAME}}.netlify.app`</Val>  auf die API.
        ...
        :::
        :::info[Github-Integration fürs Login]
        <DynamicInput name="BETTER_AUTH_GITHUB_ID" />
        <DynamicInput name="BETTER_AUTH_GITHUB_SECRET" />
        <TemplateCode>

        ```bash title="/home/dokku/{{app}}/ENV"
        dokku config:set --no-restart {{app}} BETTER_AUTH_GITHUB_ID="{{BETTER_AUTH_GITHUB_ID}}"
        dokku config:set --no-restart {{app}} BETTER_AUTH_GITHUB_SECRET="{{BETTER_AUTH_GITHUB_SECRET}}"
        ```
        </TemplateCode>
        :::
        :::info[Github-Integration fürs CMS]
        🚧 TODO: Refactor zu Better Auth 🚧
  ...
  3. SSL-Zertifikat erstellen
   ...
   4. Backups einrichten
      ...
      Falls Backups mit [dokku-keeper](https://github.com/lebalz/dokku-keeper) erstellt werden, kann `backup_config.yaml` wie folgt ergänzt werden:
      <TemplateCode>
      ```bash title="backup_config.yaml"
      {{app}}:
         files:
            - "/home/dokku/{{app}}/ENV"
         commands:
      ...
      ::::details[Backup mit S3 Speicher]
      ...
      <details>
         <summary>Backup wiederherstellen</summary>
