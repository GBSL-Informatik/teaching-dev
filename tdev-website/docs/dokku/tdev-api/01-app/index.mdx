...
<Steps>
  1. Eine neue App __<Val name="app" />__ auf dem Server erstellen
      ...
      <TemplateCode>
         ...
         dokku config:set {{app}} --no-restart NGINX_ROOT=build
         dokku config:set {{app}} --no-restart NGINX_DEFAULT_REQUEST=index.html
         dokku config:set {{app}} --no-restart DOKKU_APP_TYPE=herokuish
         dokku domains:set {{app}} {{domain}}
         # optional: indexedDB als offline-api nutzen
         dokku config:set {{app}} --no-restart OFFLINE_API="indexedDB"
         ```
      </TemplateCode>
      :::info[Mit Authentifizierung]
  ...
  2. Auf dem Laptop
     ...
     ```txt title=".buildpacks"
     https://github.com/heroku/heroku-buildpack-nodejs.git#v309
     https://github.com/dokku/buildpack-nginx.git#v28
     ```

     Mit dem ersten Buildpack wird die statische Seite generiert, mit dem zweiten Buildpack wird die generierte Seite statisch serviert. Damit dies funktioniert, muss eine
     leere Datei mit dem Namen `.static` im Root-Verzeichnis des Repositories angelegt werden.
     :::
  3. SSL-Zertifikat erstellen
     Nach der ersten Veröffentlichung, muss einmalig ein SSL-Zertifikat erstellt werden. Dies geschieht mit dem Befehl
      <TemplateCode>
      ```bash title="root@dokku"
  ...
  4. [http-auth](https://github.com/dokku/dokku-http-auth) aktivieren
     ...
     Und wieder entfernen:
      ...
      :::details[Troubleshooting: `500 Internal Server Error`]
